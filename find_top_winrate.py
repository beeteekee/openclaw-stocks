...
def main():
    ...
    # 策略1：涨幅前200名
    df_sorted = df_latest.sort_values('pct_chg', ascending=False)
    top_stocks_raw = df_sorted.head(200)['ts_code'].tolist()

    # 过滤掉北交所（.BJ）和科创板（688开头）的股票
    top_stocks = [code for code in top_stocks_raw if not code.endswith('.BJ') and not code.startswith('688.')]

    print(f"\n原始涨幅前200名股票数: {len(top_stocks_raw)}")
    print(f"过滤掉北交所和科创板后: {len(top_stocks)}只股票")
    print("="*80)

    results = []
    for i, ts_code in enumerate(top_stocks):
        print(f"[{i+1}/200] 分析 {ts_code}...", end=' ')
    ...
    print("\n" + "="*80)
    print(f"赢面最高的股票（共{len(results)}只）")
    print("="*80)
    ...
    for i, stock in enumerate(results_sorted[:10], 1):
        ...
        print(f"   赢面率: {stock['win_rate']*100:.1f}% | 综合得分: {stock['overall_score']:.1f}")
        print(f"   买点: {stock['buy_point_type']} | 250线: {stock['ma250']:.2f} | "
              f"价格>250线: {'✓' if stock['price_above_ma250'] else '✗'}")
        print(f"   长期: {stock['long_term_score']:.1f} | "
              f"中期: {stock['mid_term_score']:.1f} | "
              f"短期: {stock['short_term_score']:.1f}")
    ...
    df_result.to_csv('winrate_top_stocks.csv', index=False, encoding='utf-8-sig')
    print(f"\n已保存完整结果到 winrate_top_stocks.csv")
