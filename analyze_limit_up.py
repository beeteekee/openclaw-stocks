...
def analyze_limit_up_stocks():
    ...
    # 获取今日全市场数据
    print("正在获取今日市场数据...")
    all_stocks = pro.daily(trade_date=today, fields='ts_code,pct_chg')
    print(f"共找到 {len(all_stocks)} 只股票")
    print()

    # 筛选涨停股票
    limit_up_stocks = all_stocks[all_stocks['pct_chg'] >= 9.5]
    print(f"今日涨停股票: {len(limit_up_stocks)} 只")
    print()
    ...
    # 批量获取股票基本信息
    stock_info = pro.stock_basic(ts_code=stock_codes, fields='ts_code,name,industry')
    stock_info_dict = {row['ts_code']: row for _, row in stock_info.iterrows()}

    print(f"成功获取 {len(stock_info_dict)} 只股票的基本信息")
    print()
    ...
    for idx, stock_code in enumerate(stock_codes):
        # 显示进度
        print(f"正在分析 ({idx + 1}/{total}): {stock_code}", end='\r')

        if stock_code not in stock_info_dict:
            continue

        stock_name = stock_info_dict[stock_code]['name']
        industry = stock_info_dict[stock_code]['industry']
    ...
    print("="*60)
    print("分析完成！")
    print(f"共分析 {total} 只涨停股票，找到 {len(results)} 只符合要求的股票")
    print("="*60)
    print()
    ...
    for i, stock in enumerate(results[:10], 1):
        ...
        print(f"  市场情绪: {stock['current_emotion']}")
        print(f"  250日均线得分: {stock['ma250_score']}分")
        print(f"  缠论买点得分: {stock['chanlun_score']}分")

    return results

if __name__ == '__main__':
    top_stocks = analyze_limit_up_stocks()
